...
Chi tiết đầy đủ xem tại [Seeder README](./internal/seeder/README.md)

## 📊 Database Models

Hệ thống bao gồm 12 models chính:

- **User**: Người dùng hệ thống (với 4 role)
- **CitizenProfile**: Hồ sơ công dân (Số CCCD 12 chữ số)
- **StaffProfile**: Hồ sơ nhân viên (Liên kết phòng ban)
- **Department**: Phòng ban/Bộ phận xử lý
- **ServiceType**: Loại dịch vụ công (Với biểu mẫu JSON)
- **Application**: Đơn đăng ký dịch vụ
- **ApplicationAttachment**: Tệp đính kèm của đơn
- **ApplicationStatusLog**: Lịch sử thay đổi trạng thái đơn
- **ApplicationAssignment**: Ghi nhận phân công xử lý đơn
- **Notification**: Thông báo cho người dùng
- **ActivityLog**: Nhật ký hoạt động của hệ thống
- **ImportExportLog**: Lịch sử import/export dữ liệu

Chi tiết đầy đủ xem tại [Models Documentation](./MODELS.md)

## 🔐 Bảo Mật

- Mật khẩu được mã hóa bằng bcrypt
- Soft delete cho tất cả dữ liệu nhạy cảm
- Audit trail với `CreatedBy`, `UpdatedBy`, `DeletedBy`
- Validation đầu vào tại tất cả endpoints

## 📝 Ghi Chú Phát Triển

### Seeder Safety

- Seeder sử dụng `FirstOrCreate` để tránh trùng lặp
- Có thể chạy nhiều lần an toàn
- Không xóa dữ liệu hiện có

### CCCD Numbers

Dữ liệu mẫu sử dụng số CCCD 12 chữ số theo tiêu chuẩn Việt Nam:
